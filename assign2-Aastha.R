...
daily_power <- function(){
  ...
  demand =vector("numeric",length=15)
 ...
 #print(working_generators)
  ...
  supply = vector(mode="numeric", length=15)
  for(i in 1:nrow(station_city)){
    for(j in 1:ncol(station_city)){
    supply[i]=colSums(power_distribution)
    }
  }
  print(supply)
  
  print("(iii) difference between supply and demand :")
  diff_supplyDemand=vector("numeric",length = 15)
  for(i in 1:length(power_stations)){
    #diff_supplyDemand[i] =supply[i] - demand[i]
  }
